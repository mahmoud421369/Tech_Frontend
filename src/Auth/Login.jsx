...
import {
  ...
  FiDollarSign, FiCheckCircle, FiClock,
} from "react-icons/fi";
import api from "../api";
...
const ForgotPasswordModal = memo(({ onClose }) => {
  ...
  const handleSendCode = useCallback(async () => {
    ...
    } catch (err) { setError(sanitize(err.response?.data?.message) || "Failed to send reset code"); }
  ...
  const handleVerifyOtp = useCallback(async () => {
    ...
    try {
      ...
      setStep("reset");
    } catch (err) { setError(sanitize(err.response?.data?.message) || "Invalid code"); }
    finally { setLoading(false); }
  }, [otp, email]);
  ...
  const handleResend = useCallback(async () => {
    ...
    } catch (err) { setError(sanitize(err.response?.data?.message) || "Failed to resend"); }
    finally { setLoading(false); }
  }, [email, countdown]);
  ...
  const handleResetPassword = useCallback(async () => {
    ...
    try {
      ...
      setStep("done");
    } catch (err) { setError(sanitize(err.response?.data?.message) || "Failed to reset password"); }
    finally { setLoading(false); }
  }, [email, newPassword, confirmPw]);
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-4">
      <div className="w-full sm:max-w-sm bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/80">
          <div className="flex items-center gap-2">
            {step !== "email" && step !== "done" && (
              <button onClick={() => setStep(step === "reset" ? "otp" : "email")} className="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition">
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          {step === "email" && (
            <>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(""); }} placeholder="you@example.com" className={`${inCls} pl-10`} onKeyDown={e => e.key === "Enter" && handleSendCode()} />
              </div>
          ...
          {step === "otp" && (
            <>
              <p className="text-sm text-gray-500 dark:text-gray-400 text-center">Code sent to <span className="font-semibold text-lime-600 dark:text-lime-400 break-all">{email}</span></p>
              <div className="flex justify-center gap-1.5 sm:gap-2">
                {otp.map((d, i) => (
                  <input key={i} ref={el => inputsRef.current[i] = el} type="text" inputMode="numeric" maxLength={1} value={d} onChange={e => handleOtpChange(i, e.target.value)} onKeyDown={e => handleOtpKeyDown(i, e)} autoFocus={i === 0} className={otpCls} />
                ))}
              </div>
              <button onClick={handleVerifyOtp} disabled={loading || otp.join("").length !== 6} className="w-full py-2.5 rounded-xl bg-lime-500 hover:bg-lime-600 text-white font-bold text-sm transition disabled:opacity-70 flex items-center justify-center gap-2">
                {loading ? <><Spinner /> Verifying...</> : "Verify Code"}
              </button>
              <button onClick={handleResend} disabled={countdown > 0 || loading} className="w-full text-center text-sm text-lime-600 dark:text-lime-400 font-semibold hover:underline disabled:opacity-50 flex items-center justify-center gap-1.5">
          ...
          {step === "reset" && (
            <>
              ...
              <div className="space-y-2.5">
                <div className="relative">
                  <input type={showPw ? "text" : "password"} value={newPassword} onChange={e => { setNew(e.target.value); setError(""); }} placeholder="New password (min 6 chars)" className={`${inCls} pr-12`} />
                ...
                <input type={showPw ? "text" : "password"} value={confirmPw} onChange={e => { setConfirm(e.target.value); setError(""); }} placeholder="Confirm new password" className={inCls} />
              </div>
          ...
          {step === "done" && (
            <div className="text-center space-y-4 py-2">
              ...
              <button onClick={onClose} className="w-full py-2.5 rounded-xl bg-lime-500 hover:bg-lime-600 text-white font-bold text-sm transition">Back to Sign In</button>
            </div>
          )}
        </div>
      </div>
...
const RenewalModal = memo(({ shopId, accessToken, redirectPath, onClose, onSuccess }) => {
  ...
  const renewCard = useCallback(async () => {
    setLoading(true); setError("");
    try {
      const res = await api.post(`/api/subscriptions/renew/card/${shopId}`, { months, type: "COMMISSION" }, { headers: { Authorization: `Bearer ${accessToken}` } });
      const paymentUrl = res.data?.paymentUrl || res.data?.paymentURL;
      if (paymentUrl) {
        try {
          const parsed = new URL(paymentUrl);
          if (parsed.protocol === "https:") { setView("redirect"); setTimeout(() => { window.location.href = paymentUrl; }, 1500); }
          else setError("Unsafe payment URL received.");
        } catch { setError("Invalid payment URL received."); }
      } else {
        Swal.fire({ icon: "success", title: "Payment Confirmed!", toast: true, position: "top-end", timer: 3000, showConfirmButton: false });
        onSuccess();
      }
    } catch (err) { setError(sanitize(err.response?.data?.message) || "Card payment failed."); }
    finally { setLoading(false); }
  ...
  const renewCash = useCallback(async () => {
    setLoading(true); setError("");
    try {
      await api.post(`/api/subscriptions/cash`, { months, type: "COMMISSION" }, { headers: { Authorization: `Bearer ${accessToken}` } });
      setView("pending");
    } catch (err) { setError(sanitize(err.response?.data?.message) || "Failed to submit request."); }
    finally { setLoading(false); }
  }, [months, accessToken]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
      <div className="w-full sm:max-w-md bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        ...
        <div className="px-5 sm:px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-lime-50 dark:bg-lime-900/10 flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold text-gray-900 dark:text-white">
              {view === "form" && "Renew Subscription"}
              {view === "pending" && "Awaiting Confirmation"}
              {view === "redirect" && "Redirecting to Payment"}
            </h3>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Your subscription has expired.</p>
          </div>
          {view === "form" && (
            <button onClick={onClose} className="p-2 rounded-xl hover:bg-white/60 dark:hover:bg-gray-700 transition text-gray-500">✕</button>
          )}
        </div>
        <div className="px-5 sm:px-6 py-5 max-h-[80vh] overflow-y-auto">
          {view === "pending" && (
            <div className="text-center space-y-4 sm:space-y-5 py-2">
              <div className="w-14 h-14 sm:w-16 sm:h-16 bg-amber-100 dark:bg-amber-900/30 rounded-2xl flex items-center justify-center mx-auto">
                <FiClock size={28} className="text-amber-600 dark:text-amber-400" />
              </div>
              <div>
                <p className="text-base font-bold text-gray-900 dark:text-white">Awaiting Admin Confirmation</p>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5 leading-relaxed">Your cash payment request has been submitted successfully.</p>
              </div>
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-3.5 space-y-1.5 text-left">
                <p className="text-xs font-bold text-amber-700 dark:text-amber-400">What happens next?</p>
                <ul className="text-xs text-amber-700 dark:text-amber-400 space-y-1 list-disc list-inside">
                  <li>An admin will review and confirm your payment.</li>
                  <li>You'll receive an email once confirmed.</li>
                  <li><strong>You cannot access your shop until approved.</strong></li>
                  <li>After confirmation, simply log in again.</li>
                </ul>
              </div>
              <button onClick={() => navigate("/login")} className="w-full py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-800 transition">Back to Login</button>
            </div>
          )}
          {view === "redirect" && (
            <div className="text-center space-y-4 sm:space-y-5 py-2">
              <div className="w-14 h-14 sm:w-16 sm:h-16 bg-blue-100 dark:bg-blue-900/30 rounded-2xl flex items-center justify-center mx-auto">
                <Spinner />
              </div>
              <div>
                <p className="text-base font-bold text-gray-900 dark:text-white">Redirecting to Payment Gateway</p>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5">Please wait — <span className="font-semibold text-blue-600 dark:text-blue-400">Do not close this window.</span></p>
              </div>
            </div>
          )}
          {view === "form" && (
            <div className="space-y-4 sm:space-y-5">
              {error && <p className="text-xs text-red-600 dark:text-red-400 text-center font-medium bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-3">{error}</p>}
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3.5 sm:p-4 border border-gray-200 dark:border-gray-700">
                ...
                <div className="flex items-baseline justify-between">
                  <span className="text-xs text-gray-500 dark:text-gray-400">{PRICE_PER_MONTH} {CURRENCY} × {months}</span>
                  <span className="text-xl font-extrabold text-lime-600 dark:text-lime-400">{totalPrice.toLocaleString()} {CURRENCY}</span>
                </div>
              </div>
              <div className="flex gap-2">
                {[{ k: "card", l: "Pay by Card", icon: <FiCreditCard size={13} /> }, { k: "cash", l: "Pay Cash", icon: <FiDollarSign size={13} /> }].map(({ k, l, icon }) => (
                  <button key={k} onClick={() => setActiveTab(k)}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold border-2 transition-all ${activeTab === k ? "bg-lime-500 border-lime-500 text-white" : "border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:border-lime-400"}`}>
                    {icon}{l}
                  </button>
                ))}
              </div>
              {activeTab === "card" && (
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-3">
                  <p className="text-xs text-blue-700 dark:text-blue-400 font-medium">You'll be redirected to a secure payment gateway. Subscription activates automatically after payment.</p>
                </div>
              )}
              {activeTab === "cash" && (
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-3">
                  <p className="text-xs text-amber-700 dark:text-amber-400 font-medium">After submitting, an admin must confirm your cash payment. <strong>You cannot access your shop until confirmed.</strong></p>
                </div>
              )}
              <div className="flex gap-2.5 sm:gap-3">
                <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-700 transition">Cancel</button>
                <button onClick={activeTab === "card" ? renewCard : renewCash} disabled={loading}
                  className={`flex-1 py-2.5 rounded-xl text-white text-sm font-bold transition disabled:opacity-60 flex items-center justify-center gap-2 shadow-sm ${activeTab === "card" ? "bg-blue-600 hover:bg-blue-700" : "bg-lime-500 hover:bg-lime-600"}`}>
                  {loading ? <><Spinner /> Processing...</> : activeTab === "card" ? <><FiCreditCard size={13} /> Pay Now</> : <><FiDollarSign size={13} /> Submit</>}
                </button>
              </div>
            </div>
          )}
...
function checkLatestSubscription(subs) {
  ...
  const sorted = [...subs].sort((a, b) => (b.endDate ? new Date(b.endDate).getTime() : 0) - (a.endDate ? new Date(a.endDate).getTime() : 0));
  const latest = sorted[0];
  const endDate = latest.endDate ? new Date(latest.endDate) : null;
  return { expired: endDate !== null && endDate < new Date(), shopId: latest.shopId || latest.shop?.id || null };
}
...
const Login = ({ darkMode }) => {
  ...
  useEffect(() => { document.title = "Sign In | TechBazaar"; }, []);

  const redirectMap = useMemo(() => ({
    ROLE_ADMIN: "/admin/dashboard", ROLE_REPAIRER: "/shop/dashboard", ROLE_SELLER: "/shop/dashboard",
    ROLE_SHOP_OWNER: "/shop/dashboard", ROLE_ASSIGNER: "/assigner/dashboard",
    ROLE_DELIVERY: "/delivery/dashboard", ROLE_GUEST: "/",
  }), []);

  const SHOP_ROLES = useMemo(() => new Set(["ROLE_SHOP_OWNER", "ROLE_REPAIRER", "ROLE_SELLER"]), []);

  const validateEmail = useCallback((v) => !v.trim() ? "Email is required" : !EMAIL_REGEX.test(v) ? "Please enter a valid email" : "", []);
  const validatePassword = useCallback((v) => !v ? "Password is required" : v.length < MIN_PASSWORD_LENGTH ? `Min ${MIN_PASSWORD_LENGTH} characters` : "", []);
  ...
  const checkSubscription = useCallback(async (token) => {
    try {
      const { data } = await api.get("/api/subscriptions/all", { headers: { Authorization: `Bearer ${token}` } });
      return checkLatestSubscription(Array.isArray(data) ? data : data?.content || []);
    } catch { return { expired: false, shopId: null }; }
  }, []);

  const handleSubmit = useCallback(async (e) => {
    e.preventDefault();
    setTouched({ email: true, password: true });
    const ee = validateEmail(formData.email), pe = validatePassword(formData.password);
    if (ee || pe) { setErrors({ email: ee, password: pe, general: "" }); return; }
    setLoading(true); setErrors({ email: "", password: "", general: "" });
    try {
      const res = await api.post("/api/auth/login", { email: formData.email.trim(), password: formData.password });
      const { access_token: accessToken, role, id: backendId, email: backendEmail } = res.data;
      if (!accessToken) throw new Error("Missing access token");
      let decoded; try { decoded = jwtDecode(accessToken); } catch { /* silent */ }
      ...
      if (roles.some(r => SHOP_ROLES.has(r))) {
        ...
        if (expired) {
          Swal.fire({ icon: "warning", title: "Subscription Expired", text: "Your subscription has expired. Please renew.", toast: true, position: "top-end", timer: 5000, showConfirmButton: false, timerProgressBar: true });
          setRenewalData({ shopId, accessToken, redirectPath });
          return;
        }
      }
      await Swal.fire({ icon: "success", title: "Welcome back!", toast: true, position: "top-end", timer: 2000, showConfirmButton: false, timerProgressBar: true });
      navigate(redirectPath);
    } catch (err) {
      const msg = sanitize(err.response?.data?.message || err.message) || "Login failed.";
  ...
  const handleRenewalSuccess = useCallback(() => { navigate(renewalData?.redirectPath || "/shop/dashboard"); setRenewalData(null); }, [navigate, renewalData]);
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100">
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center w-full py-4 sm:py-0">
          ...
          <div className="space-y-5 sm:space-y-7 order-1 md:order-2">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl sm:rounded-3xl shadow-xl p-5 sm:p-7 space-y-4 sm:space-y-5">
              {errors.general && (
                <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 p-3 rounded-xl text-sm text-center font-medium" role="alert">
                  {errors.general}
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5" noValidate>
                ...
                <div className="space-y-1.5">
                  ...
                  <input id="login-email" type="email" name="email" autoComplete="email" required value={formData.email} onChange={handleChange} onBlur={handleBlur} placeholder="you@example.com" disabled={loading} className={`${ib} ${eErr ? ie : in_}`} />
                  {eErr && <p className="text-xs sm:text-sm text-red-600 dark:text-red-400 font-medium">{errors.email}</p>}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input id="login-password" type={showPassword ? "text" : "password"} name="password" autoComplete="current-password" required value={formData.password} onChange={handleChange} onBlur={handleBlur} placeholder="Enter your password" disabled={loading} className={`${ib} pr-12 ${pErr ? ie : in_}`} />
                  ...
                  {pErr && <p className="text-xs sm:text-sm text-red-600 dark:text-red-400 font-medium">{errors.password}</p>}
                </div>
              ...
              <button type="button" onClick={() => { window.location.href = "http://localhost:8080/oauth2/authorization/google"; }}
              ...
              <p className="text-center text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                New here?{" "}<Link to="/signup" className="font-semibold text-lime-600 dark:text-lime-400 hover:underline">Create an account</Link>
              </p>
            </div>
          </div>
        </div>
      ...
      {renewalData && <RenewalModal shopId={renewalData.shopId} accessToken={renewalData.accessToken} redirectPath={renewalData.redirectPath} onClose={() => { setRenewalData(null); navigate(renewalData.redirectPath); }} onSuccess={handleRenewalSuccess} />}
    </div>
  );
};
