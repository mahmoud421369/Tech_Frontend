...
const ShopChatModal = memo(({ open, onClose }) => {
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      if (err?.response?.status !== 404) setError('فشل تحميل المحادثات');
    }
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      if (msgs.length > 0) {
        api.put(`/api/chats/${activeSession.userId}/shop/${activeSession.shopId}/mark-read`).catch(() => {});
      }
  ...
  }, [open, fetchSessions, shopProfile.id]);
  ...
  const sendMessage = useCallback(() => {
    ...
    const optimistic = {
      id: optimisticId, content: clean, senderType: 'SHOP', senderName: shopProfile.email,
    ...
    setSessions(prev => prev.map(s => s.userId === activeSession.userId
      ? { ...s, lastMessage: { message: clean, sentBy: 'SHOP', createdAt: optimistic.createdAt }, unreadCount: 0 }
      : s));
    ...
    try {
      stompClient.publish({
        ...
        body: JSON.stringify({
          payload: clean, senderId: shopProfile.id, senderType: 'SHOP', sentBy: 'SHOP',
          recipientId: activeSession.userId, clientMsgId,
        }),
      });
    } catch {
        ...
        <aside className={clsx(
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar-thin">
            {isLoadingSessions ? (
              ...
              sessions.map(s => <SessionItem key={s.id ?? s.userId} session={s} isActive={activeSession?.id === s.id} onClick={() => { setActiveSession(s); setIsSidebarOpen(false); }} />)
            )}
          </div>
