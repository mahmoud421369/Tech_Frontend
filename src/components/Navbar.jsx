...
import {
  ...
  FiTruck, FiSmartphone, FiBell, FiX
} from "react-icons/fi";
        ...
        <div className="flex items-center gap-5">
            ...
            <>
              <div className="relative">
                <button
                  ...
                  className="relative p-3 rounded-xl hover:bg-emerald-500/10 transition"
                >
                  ...
                  <div className={`absolute right-0 mt-4 w-96 rounded-3xl shadow-2xl overflow-hidden border ${
                    ...
                    <div className="p-6 border-b border-emerald-500/20 flex justify-between items-center">
                      ...
                      <button onClick={() => setShowNotifications(false)}>
                        <FiX size={20} className="text-gray-500 hover:text-emerald-500" />
                      </button>
                    </div>
                          ...
                          <div key={notif.id} className={`p-5 border-b border-emerald-500/10 hover:bg-emerald-500/10 transition ${!notif.read ? "bg-emerald-500/10" : ""}`}>
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <p className={`font-semibold ${darkMode ? "text-white" : "text-gray-900"}`}>{notif.title}</p>
                                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{notif.message}</p>
                                <p className="text-xs text-emerald-500 mt-2">
                                  {new Date(notif.timestamp).toLocaleString()}
                                </p>
                              </div>
                              <button onClick={() => deleteNotification(notif.id)} className="text-red-500 hover:text-red-400">
                                <FiX size={18} />
                              </button>
                            </div>
                          </div>
              ...
              <button onClick={onCartClick} className="p-3 rounded-xl hover:bg-emerald-500/10 transition">
          ...
          {isAuthenticated && (
            <button
              ...
              className="flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold rounded-2xl hover:from-emerald-600 hover:to-teal-700 transition-all shadow-lg"
            >
              <FiLogOut size={20} />
      ...
      <nav className={`md:hidden fixed top-0 left-0 right-0 z-50 backdrop-blur-2xl py-4 px-6 border-b ${
        ...
        <div className="flex justify-between items-center">
          <Link to="/" className="flex items-center">
            <img src={logo} alt="Logo" className="h-14 rounded-xl shadow-lg" />
          </Link>
        ...
        <div className="flex justify-around">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) => `
                flex flex-col items-center gap-1 text-xs font-semibold transition-all py-2 px-3 rounded-xl
                ${isActive 
