...
const StatCard = ({ icon, value, label, accent, delay, darkMode }) => (
  <motion.div
    ...
    className={`relative group overflow-hidden rounded-xl p-3 sm:p-4 shadow-lg border transition-all duration-300 ${
      darkMode ? "bg-gray-800/80 border-gray-700/60" : "bg-white/90 border-gray-100"
    }`}
  >
...
const GlobalStyles = () => (
  <style>{`
    ...
    /* Completely hide scrollbar on tab bar */
...
const Pagination = ({ page, total, setPage, darkMode }) => (
...
const ConfirmRepairModal = ({ open, onClose, req, token, onSuccess, darkMode }) => {
  ...
  const selCls = `w-full px-4 py-3 rounded-xl border-2 text-sm font-semibold transition-all outline-none focus:ring-2 focus:ring-lime-500 focus:border-lime-500 ${
    darkMode ? "bg-gray-800 border-gray-700 text-white" : "bg-white border-gray-200 text-gray-900"
  }`;
  const optBtn = (active) =>
    `flex-1 py-2.5 rounded-xl border-2 text-xs sm:text-sm font-bold transition-all ${
      active ? "border-lime-500 bg-lime-500 text-white" : darkMode
...
const Account = ({ darkMode }) => {
  ...
  const inputCls = `w-full px-4 py-3 sm:py-3.5 rounded-xl border text-sm transition-all outline-none ${
    darkMode
  ...
  const resetAddressForm = () => { setEditingAddressId(null); setIsAddingAddress(false); setAddressForm({ state: "", city: "", street: "", building: "", notes: "", isDefault: false }); };
  const startEditAddress = (addr) => { setEditingAddressId(addr.id); setAddressForm({ state: addr.state, city: addr.city, street: addr.street, building: addr.building, notes: addr.notes || "", isDefault: addr.isDefault }); };
  ...
  const handleEditRepair = (req) => { setEditingRepair(req); setEditDescription(req.description || ""); setSelectedCategory(req.deviceCategory || ""); setIsEditRepairModalOpen(true); };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {addresses.map((addr) => (
              <motion.div key={addr.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -3 }}
                className={`relative rounded-2xl shadow-md hover:shadow-xl border-2 transition-all duration-300 overflow-hidden ${
                  addr.isDefault ? (darkMode ? "border-lime-500 bg-gray-800/80" : "border-lime-500 bg-white") : (darkMode ? "border-gray-700 bg-gray-800/80" : "border-gray-200 bg-white")
                }`}>
                ...
                <div className="p-4 sm:p-5">
                  ...
                  <div className="flex gap-2">
                    <motion.button whileTap={{ scale: 0.96 }} onClick={() => startEditAddress(addr)}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl border-2 border-lime-500 text-lime-600 dark:text-lime-400 font-bold text-xs hover:bg-lime-500 hover:text-white transition-all">
                      <FiEdit2 size={11} /> Edit
                    </motion.button>
                    <motion.button whileTap={{ scale: 0.96 }} onClick={() => handleDeleteAddress(addr.id)}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl border-2 border-red-400 text-red-500 font-bold text-xs hover:bg-red-500 hover:text-white transition-all">
                      <FiTrash2 size={11} />
                    </motion.button>
                  </div>
                </div>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {pageOrders.map((order) => {
                ...
                return (
                  <motion.div key={order.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -3 }}
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {pageRepairs.map((req) => {
                ...
                return (
                  <motion.div key={req.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -3 }}
                    className={`rounded-2xl shadow-md hover:shadow-xl border transition-all overflow-hidden flex flex-col ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
                    <div className="h-1 bg-gradient-to-r from-lime-400 to-teal-500" />
                    <div className="p-4 sm:p-5 flex flex-col flex-1">
            ...
            <div className="px-3 py-4 flex flex-col gap-1 flex-1">
              {tabs.map((tab) => (
                <motion.button key={tab.id} whileTap={{ scale: 0.97 }} onClick={() => setActiveSection(tab.id)}
                  className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl font-bold text-sm transition-all duration-200 ${
              ...
              <div className="tabs-scroll flex gap-2 overflow-x-auto pb-1 px-0.5">
                {tabs.map((tab) => (
                  <motion.button
                    ...
                    className={`flex-shrink-0 flex items-center gap-1.5 px-4 py-2.5 rounded-full font-bold text-xs sm:text-sm transition-all shadow-sm whitespace-nowrap ${
                      activeSection === tab.id
                        ...
                        : darkMode
                          ...
                          : "bg-white border border-gray-200 text-gray-700 hover:bg-lime-50"
                    }`}
                  >
                    {tab.icon}
                    {tab.label}
                    {tab.badge !== null && tab.badge !== undefined && (
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-extrabold ${activeSection === tab.id ? "bg-white/25 text-white" : "bg-lime-100 dark:bg-lime-900/40 text-lime-700 dark:text-lime-400"}`}>
                        {tab.badge}
                      </span>
                    )}
                  </motion.button>
