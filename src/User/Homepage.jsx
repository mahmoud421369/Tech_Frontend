...
const RepairIllustration = memo(({ darkMode }) => (
   <svg viewBox="0 0 200 160" className="w-full h-full">
      <circle cx="150" cy="35" r="42" fill={darkMode ? 'rgba(52,211,153,0.10)' : 'rgba(16,185,129,0.08)'} />
      ...
      <path d="M40 40 L46 26 L42 26 L48 12 L40 24 L44 24 Z" fill="#fbbf24" />
...
const OfferIllustration = memo(({ darkMode }) => (
   <svg viewBox="0 0 200 160" className="w-full h-full">
      <circle cx="150" cy="40" r="42" fill={darkMode ? 'rgba(52,211,153,0.10)' : 'rgba(16,185,129,0.08)'} />
      ...
      <path d="M138 30 L144 16 L140 16 L146 2 L138 14 L142 14 Z" fill="#fbbf24" />
      ...
      <path d="M96 100 L100 108 L109 109 L102.5 115 L104 124 L96 119.5 L88 124 L89.5 115 L83 109 L92 108 Z" fill="#fbbf24" fillOpacity="0.9" />
   </svg>
));
                              ...
                              {results.products.map(p => (
                                 <a key={p.id} href={`/device/${p.id}`}
                                    ...
                                    <div className={`w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 ${darkMode ? 'bg-white/10' : 'bg-gray-100'}`}>
                                       {p.imageUrl && <img src={p.imageUrl} alt={p.name} className="w-full h-full object-contain p-1" />}
                                    </div>
      ...
      >
        ...
        transition-all duration-200 translate-y-1 group-hover:translate-y-0">
            <button
               ...
               className={`p-1.5 sm:p-2 rounded-3xl shadow-lg backdrop-blur-sm transition-colors ${darkMode ? 'bg-gray-900/80 text-gray-200 hover:text-emerald-400'
         ...
         <div className={`relative w-full aspect-square overflow-hidden ${darkMode ? 'bg-gray-750/50' : 'bg-white'}`}>
            <ProductBackdropIllustration darkMode={darkMode} />
            <AnimatePresence>
               {!imgLoaded && !imgError && (
                  <motion.div exit={{ opacity: 0 }} transition={{ duration: 0.2 }}
                     className={`absolute inset-0 animate-pulse ${darkMode ? 'bg-gray-700' : 'bg-emerald-100'}`} />
               )}
            </AnimatePresence>
            <motion.img
               ...
               className="relative z-[1] w-full h-full object-contain p-3 border-b dark:border-gray-800 sm:p-4 group-hover:scale-[1.05]
         ...
         <div className="flex flex-col flex-1 p-3 sm:p-4 gap-1.5 sm:gap-2">
            ...
            <div className="flex flex-wrap gap-1">
               ...
               <span className="text-[10px] sm:text-[11px] font-semibold px-1.5 sm:px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">
                  {product.categoryName || product.category || 'General'}
               </span>
            </div>
            <div className="flex items-baseline gap-1.5 mt-auto pt-1">
               <span className={`text-base sm:text-lg font-extrabold tracking-tight ${darkMode ? 'text-emerald-400' : 'text-emerald-700'}`}>
      ...
      <div className="flex flex-col flex-1 p-3 sm:p-4 gap-2">
         ...
         {shop.phone && (
            <div className={`flex items-center gap-2 text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
               ...
               <a href={`tel:${shop.phone}`} onClick={(e) => e.stopPropagation()} className="hover:text-emerald-500 transition-colors truncate">
                  {shop.phone}
               </a>
            </div>
         )}
         <div className={`border-t pt-2.5 mt-auto ${darkMode ? 'border-gray-700' : 'border-gray-50'}`}>
            <motion.button
               whileTap={{ scale: 0.97 }}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
         <div className="flex items-center justify-between flex-wrap gap-3 sm:gap-4 mb-6 sm:mb-10">
            <motion.h2
               ...
               className={`text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight relative inline-block ${darkMode ? 'text-emerald-400' : 'text-emerald-800'
                  } after:content-[''] after:absolute after:bottom-[-6px] after:left-0
            after:w-12 after:h-1 after:rounded-full after:bg-gradient-to-r after:from-emerald-400 after:to-teal-500`}
            >
               {title}
            </motion.h2>
            {showArrows && (
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8">
                  <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }}
                     ...
                     <div className={`relative h-full rounded-md border p-5 sm:p-8 shadow-xl overflow-hidden flex flex-col transition-all border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
                        ...
                        <Link to="/repair" className="relative z-[1] flex flex-col h-full">
                           <div className="mb-6 sm:mb-8">
                              <div className={`inline-flex p-3.5 sm:p-4 rounded-2xl mb-4 sm:mb-5 ${darkMode ? 'bg-emerald-900/30' : 'bg-emerald-50'}`}>
                           ...
                           <div className="grid grid-cols- sm:grid-cols-1 gap-3 sm:gap-5 mb-6 sm:mb-10">
                              {[
                                 ...
                                 <div key={label} className={`flex items-center gap-3 p-3 rounded-2xl border transition-all group hover:-translate-y-0.5 ${
                  ...
                  <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }}
                     ...
                     <div className={`relative h-full rounded-md border p-5 sm:p-8 shadow-xl overflow-hidden flex flex-col transition-all border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
                        ...
                        <ul className="relative z-[1] space-y-2.5 sm:space-y-3 flex-1 mb-6 sm:mb-10">
                           {offers.map((offer, i) => (
                              <li key={i} className={`p-3 sm:p-4 rounded-2xl border flex items-center gap-3 sm:gap-4 transition-all hover:translate-x-1.5 ${
