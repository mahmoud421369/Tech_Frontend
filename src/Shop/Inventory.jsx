...
import { FiUpload, FiDownload, FiPlus, FiTrash2, FiEdit, FiSearch, FiInbox } from 'react-icons/fi';
...
const Inventory = ({ darkMode }) => {
  ...
  return (
    <div style={{ marginTop: "-550px", marginLeft: "270px" }} className="mx-auto px-4 py-8 bg-[#f1f5f9] h-auto font-cairo">
      
     
      <div className="bg-white border p-4 rounded-2xl text-right mb-4">
                    <h1 className="text-3xl font-bold text-blue-500 flex items-center justify-start flex-row-reverse gap-2"><FiInbox/>الجرد </h1>
      ...
      <div className="flex flex-wrap text-right gap-2 w-full mb-6 md:w-auto">
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-indigo-600  text-white border-2 border-indigo-500 px-4 py-2 font-bold rounded-3xl transition">
          <FiPlus /> اضافة منتج 
        </button>
        ...
        <button onClick={exportToExcel} className="flex items-center gap-2 bg-emerald-600 border-2 border-emerald-500 text-white px-4 py-2 rounded-3xl font-bold transition">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-[#f1f5f9]">
              <tr>
                ...
                <th className="px-6 py-3 text-center text-xs font-bold text-blue-600 uppercase tracking-wider">اتخاذ قرار</th>
              </tr>
            </thead>
              ...
              {currentDevices.length > 0 ? (
                currentDevices.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="p-2 font-medium whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{item.category && typeof item.category === "object" ? item.category?.name || "-" : String(item.category || "-") }</td>
                    <td className="p-2 font-medium whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">    {Number(item.price || 0).toLocaleString()} EGP
</td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 text-right">
          <h3 className="text-lg font-bold text-blue-600 dark:text-gray-300 mb-2">عدد المنتجات</h3>
          <hr className='border-gray-100 mb-4 mt-2'/>
          <p className="text-3xl font-bold text-indigo-600 dark:text-white">{stats.totalItems}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 text-right">
          <h3 className="text-lg font-bold text-blue-600 dark:text-gray-300 mb-2">منتجات الكميات القليلة</h3>
          <hr className='border-gray-100 mb-4 mt-2'/>
          <p className="text-3xl font-bold text-red-600 dark:text-red-400">{stats.lowStock}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 text-right">
          <h3 className="text-lg font-bold text-blue-600 dark:text-gray-300 mb-2">اجمالي سعر المنتجات</h3>
          <hr className='border-gray-100 mb-4 mt-2'/>
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{Number(stats.totalValue).toLocaleString()} EGP</p>
        </div>
      </div>
    </div>
  );
