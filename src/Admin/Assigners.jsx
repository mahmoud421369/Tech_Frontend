...
const Assigners = ({ darkMode }) => {
  ...
  const processed = useMemo(() => {
    ...
    let list = assigners.filter(a =>
      (statusFilter === 'all' || (a.status || '').toUpperCase() === statusFilter) &&
      (!t || (a.name || '').toLowerCase().includes(t) || (a.email || '').toLowerCase().includes(t) || (a.phone || '').toLowerCase().includes(t))
    );
    return [...list].sort((a, b) => {
      let av = String(a[sortField] || '').toLowerCase(), bv = String(b[sortField] || '').toLowerCase();
      if (['totalAssignmentsHandled', 'pendingAssignments'].includes(sortField)) { av = Number(a[sortField] || 0); bv = Number(b[sortField] || 0); }
                ...
                <table className="w-full min-w-[850px]">
                  <thead className="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                      ...
                      <Th field="totalAssignmentsHandled" label="Total Assigned" />
                      ...
                      <tr key={a.id} className="hover:bg-lime-50/10 dark:hover:bg-lime-900/5 transition-colors group">
                        <td className="px-4 py-2">
                          <div className="flex items-center gap-3">
                            ...
                            <button onClick={() => navigator.clipboard.writeText(a.id).then(() => showToast('ID Copied', 'success'))} className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-lime-500 transition-all"><FiCopy size={14} title="Copy Assigner ID" /></button>
                          </div>
                        </td>
